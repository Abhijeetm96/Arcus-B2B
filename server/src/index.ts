...
app.post('/api/admin/orders/:id/share-email', adminAuthMiddleware, async (req, res) => {
  try {
    ...
    // Render HTML & generate PDF buffer
    const htmlContent = await DocumentService.renderDocumentHtml('invoice', order);
    const pdfBuffer = await DocumentService.generatePdf(htmlContent);

    // Email body
    const emailBody = `
      <h3>Arcus Tax Invoice</h3>
      <p>Hello,</p>
      <p>Please find attached the official tax invoice for Order <strong>#${order.id}</strong>.</p>
      <p>Total Amount: <strong>₹${(typeof order.amount === 'number' ? order.amount : parseFloat(String(order.amount).replace(/[^\d.]/g, '')) || 0).toLocaleString('en-IN')}</strong></p>
      <p>Thank you for choosing Arcus!</p>
    `;

    const filename = `Invoice-${order.id.slice(-6).toUpperCase()}.pdf`;
    const previewUrl = await sendEmailWithAttachment(
      email,
    ...
    console.error('Error sharing order invoice via email:', err);
    res.status(500).json({ error: 'Failed to send email.' });
  }
});

app.post('/api/admin/bookings/:id/share-email', adminAuthMiddleware, async (req, res) => {
  try {
    const bookingId = req.params.id;
    const { email } = req.body;
    if (!email) {
    ...
    // Render HTML of the service invoice
    const htmlContent = `
<html>
<head>
  <style>
...
<body>
  ...
  <div class="total-section" style="margin-top: 35px; display: flex; justify-content: flex-end;">
    ...
    // Email body
    const emailBody = `
      <h3>Arcus Service Tax Invoice</h3>
      <p>Hello,</p>
      <p>Please find attached the official service tax invoice for Booking <strong>#${booking.id}</strong>.</p>
      <p>Total Service Amount: <strong>₹1,499.00</strong></p>
      <p>Thank you for choosing Arcus Services!</p>
    `;

    const filename = `Service-Invoice-${bookingIdSuffix}.pdf`;
    const previewUrl = await sendEmailWithAttachment(
      email,
      `Arcus Service Tax Invoice - INV-SRV-${bookingIdSuffix}`,
      emailBody,
      pdfBuffer,
      filename
    );
    ...
    console.error('Error sharing service booking invoice via email:', err);
    res.status(500).json({ error: 'Failed to send email.' });
  }
});

app.get('/api/documents/booking/:id', adminAuthMiddleware, async (req, res) => {
  try {
    const bookingId = req.params.id;
    const bookings = await getAllBookings();
    const booking = bookings.find((b: any) => b.id === bookingId) as any;
    ...
    const bookingIdSuffix = booking.id.slice(-6).toUpperCase();
    const htmlContent = await DocumentService.renderDocumentHtml('invoice', booking);
    const pdfBuffer = await DocumentService.generatePdf(htmlContent);

    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename=Service-Invoice-${bookingIdSuffix}.pdf`);
    res.send(pdfBuffer);
  } catch (err: any) {
    console.error('Error serving service booking PDF:', err);
...
app.get('/api/admin/documents/:type/:id/secure-token', adminAuthMiddleware, async (req, res) => {
  try {
    ...
    const token = encryptOrderId(`${type}:${id}`);
    const secureLink = `/api/documents/secure/${encodeURIComponent(token)}`;
    res.json({ secureLink });
  } catch (err) {
...
app.get('/api/documents/secure/:token', async (req, res) => {
  try {
    const { token } = req.params;
    const decrypted = decryptOrderId(token);
    const [type, id] = decrypted.split(':');
...
<body>
  ...
  <div class="total-section" style="margin-top: 35px; display: flex; justify-content: flex-end;">
    <table style="width: 250px; margin-top: 0; font-size: 13px;">
      ...
      const pdfBuffer = await PdfGenerator.generate(htmlContent);
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `inline; filename="Service-Invoice-${bookingIdSuffix}.pdf"`);
      ...
      const pdfBuffer = await DocumentService.generatePdf(htmlContent);
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `inline; filename="Invoice-${order.id.slice(-6).toUpperCase()}.pdf"`);
      return res.send(pdfBuffer);
    } else {
      return res.status(400).send('Invalid token type.');
    }
  } catch (err: any) {
    console.error('Error serving secure document:', err);
    res.status(500).send('Link invalid or expired.');
  }
});
