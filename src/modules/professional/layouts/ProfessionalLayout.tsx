...
export const ProfessionalLayout: React.FC<ProfessionalLayoutProps> = ({
  ...
  const menuItems = [
    ...
    { id: 'profile', name: 'Professional Profile', icon: 'account_box' },
    { id: 'verification', name: 'Verification Status', icon: 'verified' },
    { id: 'portfolio', name: 'My Portfolio', icon: 'photo_library' },
    { id: 'projects', name: 'Projects Track', icon: 'assignment_turned_in' },
      ...
      <aside className={`bg-slate-900 text-slate-200 w-64 flex-shrink-0 transition-all duration-300 flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-64 absolute md:relative'}`}>
        <div className="h-16 flex items-center justify-between px-lg border-b border-slate-800">
          <div className="flex items-center gap-xs">
            <span className="material-symbols-outlined text-[#FFC107] text-[28px] font-bold">engineering</span>
            <span className="font-extrabold text-lg tracking-wider text-white">ARCUS <span className="text-[#FFC107] text-xs font-semibold px-1.5 py-0.5 rounded bg-amber-500/10 border border-amber-500/20">PRO</span></span>
          </div>
